...
int recovery_station(int *number_station, int *user_line, int c, char *right_name)
{
     FILE *file;
    file=fopen("spisok_linii_translit.txt","r");
    if (file==NULL){printf("\n\t\t! ERROR !\nDatabase is not found. You should contact technical support.\n");  return -1;}
    ...
    for(j=0; j<4; j++)
        if(j==number) {
           ...
           while(1){
            ...
            mass_c[j]=c;
            if((recovery_station(number_station, user_line, c, wrong_name))==-1)
...
int check_answer_1(int answer, int number, char* right_name, int c, int *number_station, int *user_line)
{
      if( answer!=number ) {
                       ...
                       if((recovery_station(number_station, user_line, c, wrong_name))==-1) return -1;
                        ...
                        printf("right answer-%s\nyour answer - %s", right_name, wrong_name);
                    }
